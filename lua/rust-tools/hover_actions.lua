...
function M._run_command()
    ...
    else if M._state.ttype == "rust-analyzer.showReferences" then
...
function M.handler(_, method, result, _, _, _)
    -- taken straight from the default hover implementation
    util.focusable_float(method, function()
        ...
        vim.api.nvim_buf_set_keymap(bufnr, "n", "<Esc>", ":lua require'rust-tools.hover_actions'._close_hover()<CR>", {})

        -- clear the state when the buffer is closed
        local function onDetach(_, _) clear_state() end
        vim.api.nvim_buf_attach(bufnr, false, {on_detach = onDetach})
        ...
        vim.api.nvim_buf_set_keymap(bufnr, "n", "<CR>", ":lua require'rust-tools.hover_actions'._run_command()<CR>", {})
        -- muscle memory :)
        vim.api.nvim_buf_set_keymap(bufnr, "n", "gd", ":lua require'rust-tools.hover_actions'._run_command()<CR>", {})
        -- close on escape
        vim.api.nvim_buf_set_keymap(bufnr, "n", "<Esc>", ":lua require'rust-tools.hover_actions'._close_hover()<CR>", {})
